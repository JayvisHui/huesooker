...
act = Button(desc,text = "Activate",command=lambda: (show_frame(mainpag),opencamera()),height=3,width=15,font=("Arial",15),).grid(row=5,column=2)
...
Label(mainpag,text="HueseekerV1",bg= "#474747",fg = "white").pack()
...
def opencamera():
    ...
    dihchees.update_idletasks()
    width = dihchees.winfo_width()
    height = dihchees.winfo_height()

    resized = cv2.resize(opencv_image, (width, height))

    captureim = Image.fromarray(resized)
...
Button(gooningL,text="Quit Hueseeker",command=shutthefuckup,fg="red",bg="grey").pack(anchor="ne", padx= 15,expand=True)
...
usebut=Button(gooningL,text="Input Desired Color", command=colp,).pack(anchor='e',expand=True,fill=BOTH,padx=5,pady=1)
